...
def generate_pg_migration(spec: PlatformSpec) -> str:
    ...
    if spec.features.get("audit_chain", True):
        lines.extend([
            ...
            ");",
            (
                f"CREATE INDEX idx_{spec.platform.name}_audit_entity ON "
                f"{spec.platform.name}_audit(entity_type, entity_id);"
            ),
            (
                f"CREATE INDEX idx_{spec.platform.name}_audit_time ON "
                f"{spec.platform.name}_audit(timestamp DESC);"
            ),
            "",
        ])
